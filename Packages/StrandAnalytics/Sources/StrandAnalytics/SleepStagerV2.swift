...
public enum SleepStagerV2 {
    ...
    static func stageEpochs(_ feats: [Epoch]) -> [String] {
        ...
        for f in feats {
            ...
            // Cardiac contribution to the AWAKE emission — two orthogonal corrections COMPOSED:
            //  1. Night-level R-R sparsity (`awakeHrVarW`, our #705 port): on a PPG-derived night the
            //     HR-variability channel is intrinsically noisy, so its wake weight drops 0.8 → 0.0 for the
            //     whole night (leaving only the direct-HR `0.4·zhrv` term).
            //  2. Per-epoch motion quiescence (upstream #462): on a still wrist a raised HR / HR-variability
            //     alone must NOT promote wake, so the cardiac term is clamped to ≤ 0 (keeping only its
            //     wake-SUPPRESSING half) for THAT epoch. A moving epoch on a dense-R-R night is unchanged
            //     and stages byte-identical to upstream; either correction only ever holds wake back.
            let awakeCardiac0 = awakeHrVarW * zhvv + 0.4 * zhrv
